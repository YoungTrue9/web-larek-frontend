...
Здесь предпологается что взаимодействие между различными компонентами приложения будет осуществлятся через обмен сообщениями между вызванными событиями.
В проекте используется модель MVP (Model-View-Presenter). С помощью этой модели приложение разбивает свою структуру на три Главных основных компонентов: модель, представление, презентатор.
  1.Модель отвечает за работу с данными и бизнес логикой. (Представля собой интерфейс пользователя)
