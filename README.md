...
(Обобщающий в себе конструктор и метод привязки событий для работоспособности нашего сайта)
...
### Класс ```EventEmitter```
Класс ```EventEmitter``` Брокер событий обеспечивающий работу событий исправно. Функциональность класса стандартная: возможность установить или же снять слушатели событий, так же вызывать слушатель при возникновении события, в расширенных вариантах есть возможность подписаться на все события или слушать события по шаблону как в нашем примере.<br>
...
#### Свойства:
```POST``` - метод отправки данных на сайт.<br>
...
### Класс ```LarekApi```
Основной главный класс работующий в проекте с сетью, расширение базового класса ```Api``` на основе него cоздаем класс Apilarek.
...
#### Методы
- ```getProductList``` -- метод для получения списка товаров с сервера и передачей ей на наш сайт в 'data' на хранение
- ```orderProducts``` -- метод для отправки данных заказов на сервер и передачей ей на наш сайт в 'data' на хранение
...
#### Свойства
- ```catalog```- содержит в себе все элементы из каталога, представленные в виде Product[]
- ```basket```- содержит в себе все элементы из каталога, представленные в виде Product[], которые добавлены в корзину
- ```preview``` — для данных товара открытого в превью
- ```order```- содержит в себе данные формы IOrder
- ```formErrors```- содержит в себе данные формы FormErrors

#### Геттеры
- ```total``` - производит вычисление стоимости корзины по сумарной стоимости 
 
#### Методы
- ```setCatalog``` -- установка данных в каталог (вывод товаров на экран)
- ```setPreview``` -- установка данных на экран общие данные проекта
- ```clearBasket``` -- очистка данных корзины товары которые там лежали
- ```addToOrder``` -- добавление товара в корзину 
- ```setProductToBasket``` -- установка данных в корзину те товары которые мы заказали (чтобы отображалось что мы прибавляем новые товары и счетчик увеличивался)
- ```removeFromOrder``` -- удаление товара из заказа 
- ```removeProductToBasket``` -- удаление товара из корзины
- ```setContactField``` -- установка поля контактов покупателя для оплаты
- ```setOrderField``` -- установка поля заказа открытие модального окна в которой проходит наш заказ
- ```validateContact``` -- проверка валидации данных контакта на правильность номера и почты
- ```validateOrder``` -- проводим валидацию данных заказа что они есть вообще
- ```set total``` -- установка суммы товара в корзине на какую сумму выходят товары
- ```get statusBasket``` -- получение статуса корзины что находится в ней 
- ```get bskt``` -- получение данных товаров в корзине 
- ```get Total``` --получение сумарной суммы товаров в корзине


   ### Класс ```ProductsItem```
Класс в котором происходит хранение данных товаров каталога. <br>
(Модель хранения данных товара)<br>
Заголовок, описание, категория, изображение, цена. - то что хранится внутри карточки товара<br>
...
#### Свойства
- ```title``` - получение имени товара
- ```description``` - получение описания товара
- ```image``` - получение фотографии товара
- ```category``` - получение название категории товара
- ```id``` - получение индекс товара
- ```price``` - получение цены товара, если нету цены товара то вместо суммы будет ```null``` это прописанно в ветвлении
...
#### Конструктор: 
- принимает ```container``` с типом```act```<br>
- передает ```container``` в контейнер родителя<br>
- сохраняет элементы разметки в полях<br>
- если обьект ```act``` был передан, то создаем слушать клика и вешаем его на ```container``` в котором вызываем событие ```act```
```
...
- ```price``` -- хранение разметки цены товара
#### Методы
...
#### Типы
- ```catrgoryColor``` - Объект содержащий возможные категории для карточек и их стилистические модификаторы.
...
Поля отвечают за связь с разметкой и методы за наполнение разметки данными, то есть при клике у нас открывается большая карточка с информацией, мы дублируем информацию<br>
...
#### Конструктор
- принимает ```container``` с типом ```HTMLElement``` и опциональный обьект события ```act``` c типом ```ICardActions```
...
- передает ```container``` и ```act``` в родительский конструктор, затем сохраняет необходимые элементы разметок в полях<br>
- если с ```act``` все хорошо и был передан, то вешаем прослушку события клика на ```button``` и вызываем в нем ```act```. Сам слушатель ```container``` удаляется
...
### Класс ```CardBaskets```
Данный класс нужен для отображения карточек товаров в корзине, то есть при открытие корзины в ней будут лежать товары.<br>
Поля отвечают за связь с разметкой, методы за наполнение разметки данными, чтобы в корзине отображались данные наших товаров.<br>
...
#### Конструктор
- принимает ```container``` с типом ```HTMLElement``` и опциональный обьект события ```act``` с типом  ```ICardActions```
...
- передает ```container``` в родительский контейнер, сохраняет разметку в полях корзины<br>
- если обьект ```act``` был передан, то вешаем прослушку события клика на ```button``` c вызовом обьекта ```act```. Сам слушатель ```container``` удаляется


#### Поля 
- ```title``` --хранит разметку заголовка товара
- ```price``` --хранит разметку цены товара с ветвлением на то что у нас либо бесконечная цена либо фиксированная
- ```button``` --хранит разметку кнопки удаления их корзины
- ```index``` --хранит разметку порядкого номера товара в корзине
...
- ```catalog``` --хранит разметку каталог товаров список 
- ```wrapper``` --хранит разметку страницы (каталог, корзину, счетчик товаров на корзине)
- ```basket``` --хранит разметку кнопку корзины


#### Методы
- ```set counter``` --установка значения в счетчике товаров корзины
- ```set catalog``` --установка каталога списка товаров 
- ```set locked``` --установка класса для прокрутки страницы (блокировка при открытие модального окна с товаром, вешаем класс)
...
Здесь нету практически полей, это контейнер в котором будут выводится окна нашего сайта.<br>
Здесь имеются только поля открытия или закрытия онка, ну и так же наполнение его нужным контентом<br>
Этот класс предоставляет удобный интерфейс для создания и управления модальными окнами и так же возможность управления содержимым окна то есть контента, и обратку событий закрытия и открытия.
...
- ```total``` --хранит разметку сумарной суммы товаров
- ```items``` --хранит список товаров в корзине их количество 
...
#### Типы
- ```set phone``` --устанавливает значение поля телефона (строка под номер телефона пользователя)
- ```set email``` --устанавливает значение поля почты (строка под электронный адрес почты пользователя)
...
#### Методы
- ```onChange(field, name)``` --регистрирует событие с именем конкретного поля (подсветка поля)
- ```render(state)``` --отрисовка формы и ее элементов для отображения на сайте
...
Если все прошло хорошо и отработал Order, Contacts то выведится модальное окно с кнопкой. Будет наипсанно что заказ принят и оформлен.
...
#### Типы
- ```SuccessActions``` - описание что произошло, если все хорошо то сообщение об успехе 
