...
Веб-ларек - удобный мини-интернет магазин в котором можно просмотреть каталог товаров. Можно совершать заказы и управлять своими покупками. Проект построен на базе передовых технологий. TypeScript, HTML,SCSS. Использоваться Webpack , чтобы обеспечить вам отличный пользовательский опыт.
...
```
## 📦Сборка
Для cборки нашей программы нужно выполнить данные команды
```
npm run build
```
...
- производит соеденение входящих обьектов.
...
- ```setImage``` -- метод для инициализации изображения и описания (alt)
...
(как пример, наипсал несколько простых обьяснений что происходит. Все остальные методы работают по тому же принципу)
...
```baseUrl``` -- ссылка (url)<br>
```options``` -- запрос<br>
Методы позволяют производит обработку запроса и получение данных.


#### Методы:
- ```get``` -- метод для получения Api
- ```post``` -- метод для отправки Api
- ```handleResponse``` -- Обработчик ответа
...
#### Конструктор
- принимает ```container``` с типом ```HTMLElement``` и рпциональный обьект события ```actions``` с типом  ```ICardActions```
- 
```
interface ICardActions {
  onClick: (event: MouseEvent) => void;
...
### Класс ```Order```
Данный класс нужен для заказа в модальном окне.<br>
...
Это класс Дженерик и принимает в переменной ```T``` - это тип данных компонента для отображения
